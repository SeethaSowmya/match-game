...
class Display extends Component {
  ...
  checkingOutput = id => {
    const {displayImage, timer} = this.state
    console.log(id, displayImage)
    if (id === displayImage && timer !== 0) {
      const tempNO = Math.ceil(Math.random() * imagesList.length)
      this.setState(prevState => ({
        score: prevState.score + 1,
        displayImage: imagesList[tempNO].id,
        timer: 60,
      }))
    } else {
  ...
  render() {
    const {displayImage, activeTabId, score, status, timer} = this.state
    const filteredList = imagesList.filter(
      item => item.category === activeTabId,
    )
    const Image = imagesList.filter(each => each.id === displayImage)
    return (
      <div>
        ...
        <div className="bg">
            ...
            <>
              ...
              <ul className="ulTwo">
                {tabsList.map(each => (
                  <TabItems
                    list={each}
                    activeState={each.tabId === activeTabId}
                    tabIdChange={this.tabIdChange}
                    key={each.id}
                  />
                ))}
              </ul>
              <ul className="wrap">
                {filteredList.map(each => (
                  <ShowImages
                    key={each.id}
                    checkingOutput={this.checkingOutput}
                    details={each}
                  />
                ))}
              </ul>
          ...
          {status && (
            <div className="bgOfGameCard">
              ...
              <button type="button" className="play" onClick={this.Update}>
                <img
